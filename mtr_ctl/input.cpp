...
{
    int read_pin_button = digitalRead(pin);
    if (read_pin_button == LOW)
        return true;
    else
        return false;
}
...
void calculate_average(const int speed, const int direction){
    speed_total = speed_total - speed_readings[input_index];
    direction_total = direction_total - direction_readings[input_index];
    speed_readings[input_index] = speed;
    direction_readings[input_index] = direction;
    speed_total = speed_total + speed_readings[input_index];
    direction_total = direction_total + direction_readings[input_index];
